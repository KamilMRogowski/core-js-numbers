...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  const upper = x1 * x2 + y1 * y2;
  const lower1 = Math.sqrt(x1 ** 2 + y1 ** 2);
  const lower2 = Math.sqrt(x2 ** 2 + y2 ** 2);

  const angleInRadians = Math.acos(upper / (lower1 * lower2));
...
function getSumOfDigits(num) {
  ...
  const digitsArray = Array.from(String(number), convertNumber);
